...
// el código jamás mantiene listas de sinónimos. 'open'/'closed' son META-VALORES semánticos de pipeline
// que el executor interpreta server-side (open = etapa no terminal; closed = won|lost).
export const OPERATION_STAGES = Object.keys(OPERATION_TRANSITIONS)
export const CLOSED_STAGES: readonly string[] = ['won', 'lost']
export const PORTFOLIO_STATUSES = Object.keys(PORTFOLIO_TRANSITIONS)
const STAGE_FILTER_VALUES = [...OPERATION_STAGES, 'open', 'closed']

export type OperationClass =
  | 'list' | 'search' | 'detail' | 'count' | 'aggregate' | 'relation' | 'summarize' | 'compare' | 'explain'
  | 'create' | 'update' | 'schedule' | 'automate' | 'query'
...
export type CapabilitySpec = {
  ...
  filterValues?: Record<string, readonly string[]>  // valores CANÓNICOS declarados de filtros enumerados
...
const READ_CAPABILITIES: CapabilitySpec[] = [
  { id: 'clients.list', description: 'Listar o contar los clientes del workspace (opcionalmente por estado o búsqueda).', module: 'clients', operation: 'list', entityTypes: ['client'], requiredSlots: [], optionalFilters: ['status', 'query'], filterValues: { status: CLIENT_STATUSES }, temporal: null, reader: 'crmReadQuery:clients', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
  { id: 'clients.count', description: 'Cuántos clientes hay (agregado de conteo).', module: 'clients', operation: 'count', entityTypes: ['client'], requiredSlots: [], optionalFilters: ['status'], filterValues: { status: CLIENT_STATUSES }, temporal: null, reader: 'crmReadQuery:clients', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
  ...
  { id: 'operations.list', description: 'Listar/contar operaciones/oportunidades (por etapa; abiertas/ganadas/perdidas).', module: 'operations', operation: 'list', entityTypes: ['opportunity'], requiredSlots: [], optionalFilters: ['stage'], filterValues: { stage: STAGE_FILTER_VALUES }, temporal: { field: 'expected_close_date', supported: true }, reader: 'crmReadQuery:opportunities', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
  { id: 'operations.aggregate.value', description: 'SUMA (total) del VALOR de operaciones (p. ej. valor total de ventas ganadas en un periodo). SOLO suma total; para promedio/mínimo/máximo del valor usa crm.query (aggregate avg/min/max).', module: 'operations', operation: 'aggregate', entityTypes: ['opportunity'], requiredSlots: [], optionalFilters: ['stage'], filterValues: { stage: STAGE_FILTER_VALUES }, temporal: { field: 'expected_close_date', supported: true }, reader: 'crmReadQuery:opportunities', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
  ...
  { id: 'tasks.list', description: 'Listar/contar tareas pendientes (opcionalmente en un periodo).', module: 'tasks', operation: 'list', entityTypes: ['task'], requiredSlots: [], optionalFilters: ['priority'], filterValues: { priority: TASK_PRIORITIES }, temporal: { field: 'due_date', supported: true }, reader: 'getPendingTasks', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
  { id: 'calendar.list', description: 'Citas/agenda (hoy, esta semana, un periodo).', module: 'calendar', operation: 'list', entityTypes: ['calendar_event'], requiredSlots: [], optionalFilters: ['type'], filterValues: { type: CALENDAR_TYPES }, temporal: { field: 'date', supported: true }, reader: 'getCalendarSummary', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
  { id: 'cases.list', description: 'Trámites/expedientes (por estado, vencimiento en un periodo).', module: 'cases', operation: 'list', entityTypes: ['service_case'], requiredSlots: [], optionalFilters: ['status'], filterValues: { status: CASE_STATUSES }, temporal: { field: 'due_date', supported: true }, reader: 'crmReadQuery:service_cases', mutation: false, confirmation: 'none', freshness: 'live', permission: 'read' },
...
export function ontologyForPrompt(): string {
  return CAPABILITY_ONTOLOGY.map((c) => {
    const vals = c.filterValues
      ? ' · valores:' + Object.entries(c.filterValues).map(([k, vs]) => `${k}∈{${vs.join('|')}}`).join(' ')
      : ''
    return `${c.id} [${c.operation}${c.mutation ? '·MUTA' : ''}${c.temporal?.supported ? '·temporal' : ''}] (${c.entityTypes.join('/')}) — ${c.description}${c.requiredSlots.length ? ` · req:${c.requiredSlots.join(',')}` : ''}${vals}`
  }).join('\n')
}
