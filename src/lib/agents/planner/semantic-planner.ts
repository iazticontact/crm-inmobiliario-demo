...
- ACCIÓN EN CURSO (principio general): si el estado trae pendingAction/pendingActionContext y el mensaje del usuario APORTA datos para completarla o CORRIGE un dato ya dado, emite un goal de ESA MISMA capability poniendo en filters SOLO lo nuevo/corregido — el servidor fusiona con providedSlots y conserva la entidad ya resuelta; NO reinicies la acción ni vuelvas a pedir lo ya proporcionado. Si el usuario cambia de tema, ignora la acción pendiente (no la arrastres).
...
- FILTROS CON VALORES DECLARADOS (principio general): cuando la ontología declara los valores de un filtro (p. ej. status∈{active|lead|…}, stage∈{…|open|closed}), traduce el lenguaje del usuario al TOKEN CANÓNICO exacto («activos» → status=active; «abiertas/en curso» → stage=open; «cerradas» → stage=closed). NUNCA pongas la palabra del usuario como valor. Si ningún token encaja, omite el filtro o pide aclaración; no inventes valores.
