#!/usr/bin/env node
// P25/P26 — Verificación SEGURA del backend del Asistente (deploy o local).
//
...
//
// Uso:
//   node scripts/check-agent-deploy.mjs <URL> [--workspace <uuid>]
//   DEPLOY_URL=https://staging.example.com node scripts/check-agent-deploy.mjs
//   (URL puede ser el dominio desplegado o http://localhost:3000 para probar en local)
//
...
// `connection: close` evita que el socket keep-alive de undici deje handles
// abiertos al terminar (en Windows provocaba un assert de libuv al salir).
const BASE_HEADERS = { accept: 'application/json', connection: 'close' }
...
async function main() {
  ...
  if (!deployUrl) {
    console.error('✗ Falta la URL. Uso: node scripts/check-agent-deploy.mjs <URL> [--workspace <uuid>]')
    return 1
  }
  ...
  let diag
  try {
    const res = await fetch(`${deployUrl}/api/agent/diag`, { headers: BASE_HEADERS })
    if (!res.ok) { console.error(`✗ /api/agent/diag respondió HTTP ${res.status}`); return 1 }
    diag = await res.json()
  } catch (e) {
    console.error(`✗ No se pudo alcanzar /api/agent/diag: ${e.message}`)
    console.error('  → ¿URL correcta? ¿backend vivo? ¿dominio del CRM (no el de n8n)?')
    return 1
  }
  ...
  if (!tvOk) console.log('     ⚠ Contrato de tools desfasado: el backend no trae el último código.')
  ...
  if (!cfgOk) console.log('     ⚠ Falta alguna env (agentToolSecret/serviceRole/n8nWebhook/n8nSecret). En local es normal que n8nSecret sea false.')
  ...
  if (workspace) {
    ...
    try {
      const res = await fetch(`${deployUrl}/api/agent/diag?workspace_id=${encodeURIComponent(workspace)}`, {
        headers: { ...BASE_HEADERS, 'x-nowcrm-secret': secret },
      })
      if (res.status === 401) { console.error('✗ No autorizado: el AGENT_TOOL_SECRET local no coincide con el del backend.'); return 1 }
      probe = await res.json()
    } catch (e) {
      console.error(`✗ No se pudo sondear el workspace: ${e.message}`); return 1
    }
